...
10. **Downstream visualization analysis for regulator prioritization**
    ...
    * _**MERLIN-VIZ-based cell-cluster-specific module network visualiztion and regulator inference**_
      **MERLIN-VIZ** is an [R Shiny-based application](https://github.com/Roy-lab/MERLIN-VIZ) for visualizing MERLIN-inferred gene regulatory networks (GRNs), supporting analyses that consider all cells jointly or cells from individual clusters, and is applicable to both bulk and single-cell-derived GRNs. The application can be downloaded from the [merlin_viz_singlecell branch](https://github.com/Roy-lab/MERLIN-VIZ/tree/merlin_viz_singlecell) of the [MERLIN-VIZ GitHub repository](https://github.com/Roy-lab/MERLIN-VIZ). To initialize the application, the script [net_data_setup.R](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/MERLIN-VIZ_cellcluster_specific_module_network_visualization/net_data_setup.R) (input files requirements are mentioned in detail inside the script) must be run once together with [aux_functions.R](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/MERLIN-VIZ_cellcluster_specific_module_network_visualization/aux_functions.R) to generate the [net_data.Rdata](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/MERLIN-VIZ_cellcluster_specific_module_network_visualization/net_data.Rdata) file, which serves as the input for the MERLIN-VIZ Shiny application: [app.R](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/MERLIN-VIZ_cellcluster_specific_module_network_visualization/app.R). The following steps are required to run the MERLIN-VIZ R shiny application.
      ```text
      Rscript net_data_setup.R
      R
      > load('net_data.Rdata')
      > q()
      Rscript app.R // or Pasted the script from app.R by loading R in console
      ```
      The app looks like following:
      <img width="280" height="256" alt="image" src="https://github.com/user-attachments/assets/de6f039e-925e-401d-adae-7ac8eea08aae" />
      From the app, user can download the high quality global and cell-cluster-specific networks for a module, e.g., [Module921](https://github.com/Roy-lab/MERLIN-SUITE/tree/main/visualization/MERLIN-VIZ_cellcluster_specific_module_network_visualization/Module921).
