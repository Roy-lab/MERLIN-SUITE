...
4. **Augmented expression construction**
   <br>**Combine expression + inferred TFA:** The averaged TFA profiles (`tfa_avg_0_99.txt`) were appended to the gene expression matrix to construct an augmented input for subsequent **MERLIN-P** analysis. In parallel, the list of candidate regulators was updated to include transcription factors corresponding to the inferred TFA profiles. This augmented dataset was then used to infer gene regulatory networks (GRNs) using **MERLIN-P**.
   The input 
