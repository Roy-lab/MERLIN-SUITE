...
10. **Downstream visualization analysis for regulator prioritization**<br><br>
    ...
    * _**MERLIN-VIZ-based cell-cluster- and sample/condition-specific module network visualiztion and regulator inference**_
      <br><br>**MERLIN-VIZ** is an [R Shiny-based application](https://github.com/Roy-lab/MERLIN-VIZ) for visualizing MERLIN-inferred gene regulatory networks (GRNs). The tool supports analyses using: all cells jointly, or individual cell clusters, or individual samples/conditions, and is applicable to both bulk and single-cell-derived GRNs. The application can be downloaded from the [merlin_viz_singlecell branch](https://github.com/Roy-lab/MERLIN-VIZ/tree/merlin_viz_singlecell) of the [MERLIN-VIZ GitHub repository](https://github.com/Roy-lab/MERLIN-VIZ). To initialize the application, the script [net_data_setup.R](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/MERLIN-VIZ_module_network_visualization/net_data_setup.R) (Input file requirements are described in detail within `net_data_setup.R`) must be run once together with [aux_functions.R](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/MERLIN-VIZ_cellcluster_specific_module_network_visualization/aux_functions.R). This step generates the files [net_data_cluster.Rdata](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/MERLIN-VIZ_module_network_visualization/net_data_cluster.Rdata) (for cell-cluster-specific analyses) and [net_data_sample.Rdata](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/MERLIN-VIZ_module_network_visualization/net_data_sample.Rdata) (for sample/condition-specific analyses), which serves as the primary input files for the MERLIN-VIZ Shiny applications [app_cluster.R](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/MERLIN-VIZ_module_network_visualization/app_cluster.R) and [app_sample.R](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/MERLIN-VIZ_module_network_visualization/app_sample.R), respectively. The following steps are required to launch the MERLIN-VIZ application:
      ```text
      ...
      # Cell-cluster-specific visualization
      Rscript app_cluster.R
      
      # Sample/condition-specific visualization
      Rscript app_sample.R
      
      # Alternatively, app_cluster.R or app_sample.R can be launched directly from an interactive R session.
      ```
      Using the application, users can interactively visualize and download high-quality global, cell-cluster-specific, and sample/condition-specific module regulatory networks. Example output networks for [Module921](https://github.com/Roy-lab/MERLIN-SUITE/tree/main/visualization/MERLIN-VIZ_module_network_visualization/Module921) are provided.<br><br>
    ...
    * _**Cytoscape-based condition-specific module network visualization and regulator inference**_
      ...
      **Main Output folder**
      ...
      <br>`adj.<CONDITION>_t.txt (e.g. adj.FBSDay3_t.txt)` - adjacency matrix data of network edges per module per condition.<br><br><br>
      
      **Step-5: Creating a condition-specific network per module based on the expression matrix data**
      <br><br>Next, we relearn the condition-specific networks by measuring edge weights using both regression coefficients (`reg. prefix`) and correlation coefficients (`cc. prefix`), based on the expression data for conditions per module. MATLAB scripts used: [runcc.m](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/Cytoscape_based_condition_specific_visualization/runcc.m) (for correlation coefficients) and [runls.m](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/Cytoscape_based_condition_specific_visualization/runls.m) (for regression coefficients).
      ```text
      ...
      **Step-8: Cytoscape application**
      Now the edge file ([merged with both regression and correlation coefficient data](https://github.com/Roy-lab/MERLIN-SUITE/tree/main/visualization/Cytoscape_based_condition_specific_visualization/module_921/merged_networks)) per edge is loaded in Cytoscape, with node attributes as pseudobulk expression for each condition in a [network style file](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/Cytoscape_based_condition_specific_visualization/last_styles6_3shapes.xml).<br>
      **Main Output**
      <br> **Cytoscape figure file:** [condition_specific_gene_regulatory_networks.cys](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/Cytoscape_based_condition_specific_visualization/condition_specific_gene_regulatory_networks.cys)<br>
      <br> **Individual condition specific files:** saved indiviually in .svg figure format from Cytoscape figure file.
      <br> [MEFDay0.svg](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/Cytoscape_based_condition_specific_visualization/MEFDay0.svg)
      <br> [FBSDay3.svg](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/Cytoscape_based_condition_specific_visualization/FBSDay3.svg)
      <br> [FBSDay6.svg](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/Cytoscape_based_condition_specific_visualization/FBSDay6.svg)
      <br> [FBSDay9.svg](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/Cytoscape_based_condition_specific_visualization/FBSDay9.svg)
      <br> [FBSDay12.svg](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/Cytoscape_based_condition_specific_visualization/FBSDay12.svg)
      <br> [A2SDay2.svg](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/Cytoscape_based_condition_specific_visualization/A2SDay2.svg)
      <br> [A2SDay4.svg](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/Cytoscape_based_condition_specific_visualization/A2SDay4.svg)
      <br> [A2SDay6.svg](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/Cytoscape_based_condition_specific_visualization/A2SDay6.svg)
      <br> [FBSmESCs.svg](https://github.com/Roy-lab/MERLIN-SUITE/blob/main/visualization/Cytoscape_based_condition_specific_visualization/FBSmESCs.svg) <br><br><br>
