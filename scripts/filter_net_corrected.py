...
def main():
    ...
    parser.add_argument('--filter-gold-only', action='store_true')
    ...
    if args.inferred_TFs is None: #fixed
        inferred_TFs, inferred_targets = get_TFs_and_targets(inferred)
    else:
        inferred_TFs = set([l.rstrip() for l in args.inferred_TFs.readlines()])
        inferred_targets = set([l.rstrip() for l in args.inferred_targets.readlines()])

    # TODO: check, should we intersect gold and inferred before filtering?
    if not args.filter_gold_only:
        inferred = filter_shared_nodes(inferred, gold_TFs, gold_targets)
    gold = filter_shared_nodes(gold, inferred_TFs, inferred_targets) # may differ from saptarshi's
